...
2. 未解除routeをすべて解除し、全BrowserContextをcloseする。
3. 録画が有効な場合は動画を確定または削除する。

各 teardown は自身の cleanup を全件試行する。複数の処理が失敗した場合は、全原因を `AggregateError` で伝播する。
生成数と解放数の台帳照合は置かない。資源リークの検知は Playwright fixture の破棄に委ねる。
...
通常 E2E は直列（`workers: 1`、`fullyParallel: false`）を維持する。デモ規模（十数シナリオ・単一ホスト）では Chromium 直列の wall time が運用上許容できる。並列のメリットは主にマルチエンジン必須運用向けであり、本デモは該当しない。bench 基盤は撤去済みである。今後の時間計測は Playwright 標準の実行結果サマリーで足りる。詳細な数値表は git 履歴の `docs/report.md`（例: commit `8f7c21c`）で追える。

成功時は診断 artifact を残さない。失敗時は Playwright 標準成果物で調査する。

- スクリーンショット（`only-on-failure`）
- シナリオが作成した全 BrowserContext を含む trace（`retain-on-failure`）
- 失敗時に添付する console / page error / stack log（Playwright attachment）
- HTML report（`playwright-report/`。ローカルで `npx playwright show-report` を使う）

失敗した assertion の actual / expected / scenario context は、テスト失敗メッセージ（`formatAssertionFailure`）に残る。
trace の Console、Errors、Network タブで console、page error、通信内容を確認する。追加contextは `E2eContext.withSession` で作成する。

CI は GitHub annotation と JUnit XML でケース件数と失敗ケースを表示する。CI が失敗した場合は `test-results/` と `playwright-report/` を artifact として保存する。
...
| `session.ts`         | managed session・fixtureのsetup/teardown                            |
