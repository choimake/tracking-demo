...
シナリオ teardown のブラウザ不要な回帰チェックは `npm run e2e:scenario-teardown-check` で実行する。資源台帳の回帰チェックは `npm run e2e:managed-session-check` で実行する。
...
`expectHitCountAtMost` は現在、回帰契約のみで使用する。回帰契約の検証対象として維持する。
...
`npm run quality` は Node レベルの相関・assertion回帰チェックを自動実行する。単独で確認する場合は次のコマンドを実行する。
...
`scenarios.ts` は通常 E2E のシナリオ登録である。`bench/suite-worker.ts` も同じ登録を読む。
