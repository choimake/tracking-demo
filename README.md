...
- cookie / localStorage 等によるユーザー・セッション識別はしません(ヒットは ws・url・ua・ts 等のみです)。
...
npm run e2e        # ブラウザE2E(要 npm start、初回は npx playwright install chromium-headless-shell)
```
...
Chromium / Firefox / WebKit の3エンジンを直列 headless 実行し、以下の17項目を実機検証します
...
- テスト発火(▷)は実際の受信経路(`/api/collect`)を通る `test:true` のヒットとして記録し、計測件数・レポートには含めません。
- 計測件数の「直近7日間」は、サーバーのローカル日付で数えた7暦日に統一しています(イベント一覧とレポートで同じ集計窓です)。

## 本番化時の課題(このデモで割り切った点)

デモ範囲の外(実 GTM・ユーザー識別など)は上記「やっていないこと」を参照してください。ここでは本番非機能の割り切りのみを挙げます。
...
- CSP 等のセキュリティヘッダーはなく、HTTP のみです(本番は HTTPS/CSP 前提です)。
